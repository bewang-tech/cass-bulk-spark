 ...
 *   // CQLSSTableWriterExt.Builder for more details on the available options.
 *   CQLSSTableWriterExt writer = CQLSSTableWriterExt.builder()
 ...
 * </pre>
 */
public class CQLSSTableWriterExt implements Closeable {
    static {
        Config.setClientMode(true);
    }

    static class Operation<T extends ModificationStatement> {
        public Operation(T statement, List<ColumnSpecification> boundNames) {
    ...
    private final Operation<UpdateStatement> insert;
    private final Operation<DeleteStatement> deleteRow;
    private final Operation<DeleteStatement> deletePartition;

    private CQLSSTableWriterExt(AbstractSSTableSimpleWriter writer,
                             Operation<UpdateStatement> insert,
                             Operation<DeleteStatement> deleteRow,
                             Operation<DeleteStatement> deletePartition) {
        this.writer = writer;
        this.insert = insert;
    ...
    /**
     * Returns a new builder for a CQLSSTableWriterExt.
     *
    ...
    public CQLSSTableWriterExt deleteRow(Object... values)
            throws InvalidRequestException, IOException {
        return rawDelete(deleteRow, getRawData(deleteRow, Arrays.asList(values)));
    }

    public CQLSSTableWriterExt deletePartition(Object... values)
            throws InvalidRequestException, IOException {
        return rawDelete(deletePartition, getRawData(deletePartition, Arrays.asList(values)));
    }

    private <T extends ModificationStatement> List<ByteBuffer> getRawData(Operation<T> op, List<Object> values) {
        int size = Math.min(values.size(), op.boundNames.size());
        List<ByteBuffer> rawValues = new ArrayList<>(size);
        for (int i = 0; i < size; i++)
            rawValues.add(values.get(i) == null ? null : ((AbstractType) op.boundNames.get(i).type).decompose(values.get(i)));
        return rawValues;
    }

    private CQLSSTableWriterExt rawDelete(Operation<DeleteStatement> op, List<ByteBuffer> values)
            throws InvalidRequestException, IOException {
        if (values.size() != op.boundNames.size())
            throw new InvalidRequestException(String.format("Invalid number of arguments, expecting %d values but got %d", op.boundNames.size(), values.size()));

        DeleteStatement stmt = op.statement;
        QueryOptions options = QueryOptions.forInternalCalls(null, values);
        List<ByteBuffer> keys = stmt.buildPartitionKeyNames(options);
        Composite clusteringPrefix = stmt.createClusteringPrefix(options);

        long now = System.currentTimeMillis() * 1000;
        UpdateParameters params = new UpdateParameters(stmt.cfm,
                options,
                stmt.getTimestamp(now, options),
        ...
        try {
            for (ByteBuffer key : keys) {
                if (writer.shouldStartNewRow() || !key.equals(writer.currentKey().getKey()))
                    writer.newRow(key);
                stmt.addUpdateForKey(writer.currentColumnFamily(), key, clusteringPrefix, params);
            }
            return this;
     ...
     * @return this writer.
     */
    public CQLSSTableWriterExt addRow(Object... values)
            throws InvalidRequestException, IOException {
     ...
     * @return this writer.
     */
    public CQLSSTableWriterExt addRow(List<Object> values)
            throws InvalidRequestException, IOException {
        int size = Math.min(values.size(), insert.boundNames.size());
        List<ByteBuffer> rawValues = new ArrayList<>(size);
        for (int i = 0; i < size; i++)
            rawValues.add(values.get(i) == null ? null : ((AbstractType) insert.boundNames.get(i).type).decompose(values.get(i)));
     ...
     * @return this writer.
     */
    public CQLSSTableWriterExt addRow(Map<String, Object> values)
            throws InvalidRequestException, IOException {
        int size = insert.boundNames.size();
        List<ByteBuffer> rawValues = new ArrayList<>(size);
        for (int i = 0; i < size; i++) {
            ColumnSpecification spec = insert.boundNames.get(i);
     ...
     * @return this writer.
     */
    public CQLSSTableWriterExt rawAddRow(ByteBuffer... values)
            throws InvalidRequestException, IOException {
     ...
     * @return this writer.
     */
    public CQLSSTableWriterExt rawAddRow(List<ByteBuffer> values)
            throws InvalidRequestException, IOException {
        if (values.size() != insert.boundNames.size())
            throw new InvalidRequestException(String.format("Invalid number of arguments, expecting %d values but got %d", insert.boundNames.size(), values.size()));

        UpdateStatement stmt = (UpdateStatement) insert.statement;
        QueryOptions options = QueryOptions.forInternalCalls(null, values);
        List<ByteBuffer> keys = stmt.buildPartitionKeyNames(options);
        Composite clusteringPrefix = stmt.createClusteringPrefix(options);
        ...
        try {
            for (ByteBuffer key : keys) {
                if (writer.shouldStartNewRow() || !key.equals(writer.currentKey().getKey()))
                    writer.newRow(key);
                stmt.addUpdateForKey(writer.currentColumnFamily(), key, clusteringPrefix, params, false);
            }
            return this;
        } catch (BufferedWriter.SyncException e) {
            // If we use a BufferedWriter and had a problem writing to disk, the IOException has been
     ...
     * @return this writer.
     */
    public CQLSSTableWriterExt rawAddRow(Map<String, ByteBuffer> values)
            throws InvalidRequestException, IOException {
        int size = Math.min(values.size(), insert.boundNames.size());
        List<ByteBuffer> rawValues = new ArrayList<>(size);
        for (int i = 0; i < size; i++) {
    ...
    /**
     * A Builder for a CQLSSTableWriterExt object.
     */
    public static class Builder {
        ...
        private Operation deleteRow;
        private Operation deletePartition;
        ...
        public Builder forTable(String schema) {
            try {
                synchronized (CQLSSTableWriterExt.class) {
        ...
        public Builder using(String insertStatement) {
            ...
            this.insert = getOperation(insertStatement, UpdateStatement.class, "INSERT");
            return this;
        }

        private void validate(Operation op) {
            if (op.statement.hasConditions())
                throw new IllegalArgumentException("Conditional statements are not supported");
            if (op.boundNames.isEmpty())
                throw new IllegalArgumentException("Provided insert statement has no bind variables");
        }

        public Builder forUpdate(String updateStatement) {
            return using(updateStatement);
        }

        public Builder forDeleteRow(String deleteStatement) {
            if (schema == null)
                throw new IllegalStateException("You need to define the schema by calling forTable() prior to this call.");

            this.deleteRow = getOperation(deleteStatement, DeleteStatement.class, "DELETE");
            return this;
        }

        public Builder forDeletePartition(String deleteStatement) {
            if (schema == null)
                throw new IllegalStateException("You need to define the schema by calling forTable() prior to this call.");

            this.deletePartition = getOperation(deleteStatement, DeleteStatement.class, "DELETE");
            return this;
        }
        ...
        /**
         * Creates a CQLSSTableWriterExt that expects sorted inputs.
         * <p>
        ...
        private <T extends ModificationStatement> Operation<T> getOperation(String query, Class<T> klass, String type) {
            Pair<T, List<ColumnSpecification>> pair = getStatement(query, klass, type);
            Operation<T> op = new Operation(pair.left, pair.right);
            validate(op);
            return op;
        }
        ...
        public CQLSSTableWriterExt build() {
            if (directory == null)
            ...
            return new CQLSSTableWriterExt(writer, insert, deleteRow, deletePartition);
        }
    }

    /**
     * CQLSSTableWriterExt doesn't use the method addColumn() from AbstractSSTableSimpleWriter.
